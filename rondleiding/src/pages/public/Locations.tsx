...
export function LocationsPage() {
    ...
    const floors = useMemo(
        () =>
            Array.from(
                new Set(locations.map((item) => item.floor).filter(Boolean)),
            ).sort() as string[],
        [locations],
    );
    ...
    const filteredLocations = useMemo(() => {
        ...
        return locations.filter((location) => {
            const matchesQuery =
                !q ||
                location.name.includes(q) ||
                location.description.includes(q);
            const matchesBuilding =
                buildingFilter === "all" ||
                location.buildingId === buildingFilter ||
                location.buildingName === buildingFilter;
            const matchesFloor =
                floorFilter === "all" || location.floor === floorFilter;
                ...
                <div className="roc-content-wrap roc-band">
                    <div>
                        ...
                        <h1 className="roc-title mt-3">
                            Zoek en filter locaties binnen de school
                        </h1>
                        <p className="roc-copy mt-4">
                            De filterstrook volgt de diagonale en strakke
                            lay-out uit de huisstijl.
                        </p>
                    </div>

                    <section className="roc-panel z-index-10 grid gap-4 p-5 sm:grid-cols-3 md:p-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <select
                                ...
                                className="h-11 w-full border border-border bg-background px-3 text-sm outline-none transition focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/30"
                            >
