...
export function RouteDetail() {
        ...
        const enrichedRoute: GuideRoute = {
          ...
          locations: result.locations?.map((step) => {
            ...
            return {
              ...
              imageUrl: (location as any)?.imageUrl || 'https://images.unsplash.com/photo-1582407947304-fd86f028f716?q=80&w=1000&auto=format&fit=crop', 
            };
          }),
        };
  ...
  if (loading) return <div className="flex h-screen items-center justify-center bg-white font-sans text-[#333333]">route laden...</div>;
  if (error) return <div className="flex h-screen items-center justify-center bg-white font-sans text-[#C3071B] font-bold">{error}</div>;
  ...
  return (
    <main className="relative flex h-screen flex-col overflow-hidden bg-[#004B98] font-sans text-[#333333] selection:bg-[#0064AD]/20">
      
      {/* BOVENSTE HELFT: Grote full-screen foto met knoppen er bovenop */}
      <section className="relative w-full h-[30vh] bg-neutral-900 overflow-hidden">
        
        {/* Sluitknop */}
        <Button asChild variant="ghost" size="icon" className="absolute top-6 right-6 z-20 rounded-full bg-black/30 text-white hover:bg-black/50 backdrop-blur-md">
          <Link to="/routes">
            <X className="size-6" />
          </Link>
        </Button>
        ...
        {/* Diagonale ROC Twente-stijlkenmerk driehoek-overlay */}
        <div className="absolute inset-0 bg-gradient-to-t from-[#004B98] via-transparent to-black/20 pointer-events-none" />
        <div 
          className="absolute inset-0 bg-[#E3001A]/10 pointer-events-none"
          style={{ clipPath: 'polygon(0 0, 100% 0, 100% 100%, 25% 100%)' }}
        />
      </section>

      {/* ONDERSTE HELFT: Lyrics Sheet van de HUIDIGE stap */}
      <section className="relative flex h-[70vh] flex-col bg-gradient-to-b from-[#004B98] to-[#0064AD] p-6 text-white shadow-[0_-15px_40px_rgba(0,0,0,0.3)] rounded-t-[32px] z-10 overflow-hidden">
        ...
        <div className="mb-4 flex items-end justify-between border-b border-white/10 pb-4">
          ...
          >
            {isPlaying ? <Pause className="size-6 fill-white" /> : <Play className="size-6 fill-white ml-1" />}
          </Button>
        </div>

        {/* SCROLLBARE CONTAINER: Alleen voor de tekst van de actieve stap */}
        <div 
            ...
            <span className="flex flex-wrap gap-x-[0.23em] gap-y-2">
              {words.map((word, wordIdx) => {
                ...
                const dynamicStyle = isPlaying 
                  ? (isWordSpeaking ? 'opacity-100 text-[#FFF265] scale-105 drop-shadow-[0_0_12px_rgba(255,242,101,0.6)]' : 'opacity-30 text-white/50') 
        ...
        <div className="mt-4 flex items-center justify-between border-t border-white/10 pt-4">
          <Button
            ...
            className="text-white/70 hover:text-white hover:bg-white/10 rounded-[2mm] text-sm font-semibold gap-1 lowercase"
          >
            <ChevronLeft className="size-4" /> vorige
          </Button>
          
          <span className="text-xs text-white/40 font-sans truncate max-w-[140px] lowercase">
            {route.name}
          </span>
